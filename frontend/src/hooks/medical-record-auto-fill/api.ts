...
const mapSectionsToRecord = (sections: Array<{ nombre: string; textoActual?: string | null; textoSugeridoIa?: string | null }>) => {
  const next = { ...emptyRecord };
  for (const section of sections) {
    if (section.nombre in next) {
      (next as any)[section.nombre] = section.textoActual || section.textoSugeridoIa || "";
    }
  }
  return next;
};
...
const waitForQueuedRecord = async (consultaId: string, controller: AbortController): Promise<MedicalRecordData | null> => {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    const hasPendingIa = sections.some((section: any) =>
