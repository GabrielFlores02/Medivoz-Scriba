...
interface UseSessionRecorderReturn {
  ...
  handleStartRecording: (preparedSession?: {
    sessionId: string;
    dbSessionId: string;
  }) => Promise<boolean>;
...
export function useSessionRecorder({
  ...
  const generateSessionId = async (): Promise<{ sessionId: string; dbSessionId: string } | null> => {
    ...
    try {
      const response = await api.post("/clinical/consultations", {
        ...
        especialidadId: specialtyId ?? null,
  ...
  const handleStartRecording = async (preparedSession?: {
    sessionId: string;
    dbSessionId: string;
  }) => {
    ...
    let currentDbId = preparedSession?.dbSessionId || dbSessionId;
    let currentCode = preparedSession?.sessionId || sessionId;
