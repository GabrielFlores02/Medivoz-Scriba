...
import { specialities } from "../db/schema/auth.js";
...
import { studyConsultations } from "../db/schema/study.js";
...
const buildPrompt = (
  ...
  const previousBlock = sections
    ...
    .join("\n");
  const specialtyBlock = specialtyContext
    ? `
Contexto de especialidad: ${specialtyContext.specialty}.
Vocabulario de apoyo validable: ${specialtyContext.terms.join(", ") || "sin terminos configurados"}.
Secciones orientativas de plantilla: ${specialtyContext.templateSections.join(", ") || "plantilla base"}.
El vocabulario sirve solo para reconocer palabras pronunciadas; nunca autoriza a completar datos ausentes.
`
    : "";
...
- Extrae exclusivamente anamnesis subjetiva.
- Excluye examen fisico, examen mental estructurado, diagnostico, discusion diagnostica, plan,
  tratamiento, indicaciones, recetas, examenes auxiliares, procedimientos y cierre.
- En Psiquiatria conserva solo el relato subjetivo verbalizado; no construyas un examen mental completo.
...
export const clinicalWorker = new Worker<ClinicalExtractionJobData>(
  ...
  async (job) => {
    ...
    const studyContext = await db.query.studyConsultations.findFirst({
      columns: {
        especialidad: true,
        flujo: true,
      },
    ...
    const [consultationSpeciality] = await db
      .select({ nombre: specialities.nombre })
      .from(consultations)
      .leftJoin(
        specialities,
        eq(consultations.especialidadId, specialities.id)
      )
      .where(eq(consultations.id, consultaId))
      .limit(1);
    const selectedSpeciality =
      studyContext?.especialidad || consultationSpeciality?.nombre || null;
    const specialtyKnowledge =
      studyContext?.flujo === "asistido"
        ? await studyService.getSpecialtyKnowledge(studyContext.especialidad)
        : null;
    const specialtyPromptContext = selectedSpeciality
      ? {
          specialty: selectedSpeciality,
          terms:
            specialtyKnowledge?.dictionary?.estado === "validado"
              ? specialtyKnowledge.terms
                  .map((term) => term.terminoCanonico)
                  .filter(Boolean)
                  .slice(0, 120)
              : [],
          templateSections:
            specialtyKnowledge?.template?.estado === "validado" &&
            Array.isArray(specialtyKnowledge.template.secciones)
            ? specialtyKnowledge.template.secciones
